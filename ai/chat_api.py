...
def _safe_log_chatbot_interaction(*args, **kwargs) -> None:
    """Never let audit logging fail the user-facing chat response."""
    try:
        log_chatbot_interaction(*args, **kwargs)
    except Exception:
        pass
...
def process_chat_message(user_query: str, history: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    # Always return rows + chart when SQL succeeded; LLM summary/recs are best-effort.
    try:
    ...
    return {
        ...
        "rows": rows,
        "chart": chart,
        "followups": (recommendations.get("followups") or [])[:5],
        "business_advice": (recommendations.get("business_advice") or [])[:5],
