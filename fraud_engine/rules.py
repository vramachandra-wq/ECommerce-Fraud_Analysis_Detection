...
def _get_delay_minutes(cursor: Any, rule_id: str, fallback_value: int = 60) -> int:
    """Fetches and caches the hold-delay (minutes) from master.rule_master."""
    if rule_id not in _DELAY_CACHE:
        ...
        if row and row[0] is not None:
            ...
            _DELAY_CACHE[rule_id] = int(fallback_value)
...
def check_r001(cursor: Any, ctx: Dict[str, Any]) -> Tuple[bool, Optional[str]]:
    ...
    if ctx["program_id"] == "P2" and "iphone 16" in (ctx.get("product_name") or "").lower():
        delay_minutes = _get_delay_minutes(cursor, "R001", fallback_value=180)
        return True, f"R001: iPhone 16 order on P2 track — held for {delay_minutes} minute review window"
    return False, None
