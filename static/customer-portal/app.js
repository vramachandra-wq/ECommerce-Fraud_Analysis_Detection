...
function uniqueCategories(products) {
  // Vouchers are shown in a dedicated rail below View cart — not as a chip.
  const cats = [
    ...new Set(
      (products || [])
...
function shopFilterRowHtml(categories, active, hasVouchers) {
  const voucherActive = active === "Vouchers";
  const voucherChip = hasVouchers
    ? `<button
          type="button"
          class="category-chip category-chip-voucher ${voucherActive ? "active" : ""}"
          data-category="Vouchers"
          role="tab"
          aria-selected="${voucherActive ? "true" : "false"}"
  ...
  return `
      <section class="filter-panel filter-panel-categories" aria-label="${esc(t("filter_by_category"))}">
        ...
        <div class="category-filter" role="tablist">${categoryChipsHtml(categories, voucherActive ? "" : active)}</div>
      </section>
      ${
        hasVouchers
          ? `<section class="filter-panel filter-panel-vouchers" aria-label="${esc(t("shop_cat_vouchers") || "Vouchers")}">
              <p class="filter-panel-label">${esc(t("shop_vouchers_label") || "Vouchers")}</p>
              <div class="category-filter" role="tablist">${voucherChip}</div>
            </section>`
          : ""
      }`;
}

function shopViewCartButtonHtml() {
  const count = cartCount();
  const badge = count
    ? `<span class="nav-badge" aria-label="${esc(t("shop_cart_hint", { count }))}">${esc(count > 99 ? "99+" : count)}</span>`
    : "";
  return `<a class="btn btn-secondary shop-view-cart" href="#/cart">${esc(t("view_cart"))}${badge}</a>`;
}
...
async function renderOrder() {
  ...
  try {
    ...
    const voucherPanel = hasVouchers
      ? `<section class="filter-panel filter-panel-vouchers" aria-label="${esc(t("shop_cat_vouchers") || "Vouchers")}">
          <p class="filter-panel-label">${esc(t("shop_vouchers_label") || "Vouchers")}</p>
          <div class="category-filter" role="tablist">
            <button
              type="button"
              class="category-chip category-chip-voucher ${shopCategory === "Vouchers" ? "active" : ""}"
              data-category="Vouchers"
              role="tab"
              aria-selected="${shopCategory === "Vouchers" ? "true" : "false"}"
            >${esc(t("shop_cat_vouchers") || "Vouchers")}</button>
          </div>
        </section>`
      : "";
    ...
    const main = `
      ${pageHead(t("shop_browse"), t("shop_browse_lede"))}
      <div class="shop-top${hasVouchers ? "" : " shop-top--no-vouchers"}">
        ...
        <div class="shop-filter-row">
          <section class="filter-panel filter-panel-categories" aria-label="${esc(t("filter_by_category"))}">
            ...
            <div class="category-filter" role="tablist">${categoryChipsHtml(
              categories,
              shopCategory === "Vouchers" ? "" : shopCategory,
            )}</div>
          </section>
          ${voucherPanel}
        </div>
    ...
    function syncCategoryChips() {
      ...
      // When Vouchers is selected, keep Categories "All" visually inactive
      // (handled by active === chip dataset). When a product category is selected,
      // voucher chip is inactive.
    }
    ...
    function refreshProductView({ resetPage = false } = {}) {
      ...
      const cartBtn = document.querySelector(".shop-view-cart");
      if (cartBtn) cartBtn.outerHTML = shopViewCartButtonHtml();
